...
&emsp; &emsp; `"stack"` : 比較大類總數 <br>
&emsp; &emsp; `"dodge"` : 比較各細類數量 <br>
&emsp; &emsp; `"fill"` :  比較細類比率<br>
</p class="wwl"><br>
...
根據以上圖表
  ...
  + 你知道哪個性別有較多的「好人」角色嗎？ 男性
  + 「男性中立角色要比女性中立角色來得多」，這句話是公平的嗎？ 不公平，因為比例不代表實際數字
...
+ 依照性別分類，比較「中立」角色的數量 dodge
+ 依照性別分類，比較「中立」角色的比例 fill
+ 比較各性別的總人數 stack
...
```{r  fig.height=1.8, fig.width=7, echo=F}
gg = lapply(c("stack","dodge","fill"), function(pos) {
  ggplot(dx, aes(align,n,fill=sex)) + 
    geom_col(position=pos, alpha=0.6) + labs(title=pos,y="")
  })
...
+ 繪製**數字**而不是**比率**嗎？ 改成position="stack"
+ 你發現了什麼呢？ 比例與趨勢與數量可能落差很大
+ 在這個 exercise 中，你學到了什麼呢？ 除了比例也要看數量
```{r}
hx = count(D2, hair, sort=T)
ex = count(D2, eye, sort=T)
D2 %>% filter(
