...
```{r}
mean(df$height[df$isMale]) # to caculate the average of males' height
```

```{r}
df$height[!df$isMale] %>% mean # the mean of female height
```

```{r}
sum( subset(df, !isMale)$noBuy ) # total number of noBuy from all female
```
