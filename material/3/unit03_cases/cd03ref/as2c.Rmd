...
```{r}
tapply(A$Country=="India", A$MetroArea, sum, na.rm=T) %>% sort %>% tail
```
<br>

⏰ <z>請注意</z> ⏰
這個題目如果不放`na.rm=T`，運算的結果會變成 
```{r}
tapply(A$Country=="India", A$MetroArea, sum) %>% sort %>% tail
```
本來人數最多的`Long Island`,`Alexandria`,`Wilmington`為什麼都不見了呢 ❓ <br><br>

`Long Island`裡面確實是有`96`個受訪者的出生地是在`India`
```{r}
loc1 = subset(A, MetroArea=="New York-Northern New Jersey-Long Island, NY-NJ-PA")
# sum(is.na(loc1$Country)) 
table( loc1$Country == "India", useNA="always" )
```

但是因為這個地區的`Country`裡面有缺項，所以`A$Country=="India"`的向量也會帶`NA`，不用`na.rm=T`這個選項的話，`sum()`預設就是`NA`，所以`tapply()`的輸出向量裡面`Long Island`的值也會是`NA`，`sort()`過之後它就看不見了；`Alexandria`,`Wilmington`,`Joliet`也都是同樣的問題。

`Detroit-Warren-Livonia, MI`的受訪者裡面，`Country`都沒有`NA`
```{r}
loc2 = subset(A, MetroArea=="Detroit-Warren-Livonia, MI")
table( loc2$Country == "India", useNA="always" )
```
所以它是<u>受訪者的`Country`沒有缺項的地區中</u>，`A$Country=="India"`最多的地方，但這不是我們要的答案。所以說： <br>
⏰ 資料中存在`NA`時，呼叫`sum()`,`mean()`這一類統計功能記得要放`na.rm=T`是很重要的。<br>

這一個題目也可以這樣做，
```{r}
A$MetroArea[A$Country=="India"] %>% table %>% sort %>% tail
```
因為條件式索引`[A$Country=="India"]`本身就會排除`NA`，而且`table()`的預設值也不會計算`NA`，所以就不用放`na.rm=T`了。
<br>


In Brazil?
```{r}
# tapply(A$Country=="Brazil", A$MetroArea, sum, na.rm=T) %>% sort %>% tail
A$MetroArea[A$Country=="Brazil"] %>% table %>% sort %>% tail
```

In Somalia?
```{r}
# tapply(A$Country=="Somalia", A$MetroArea, sum, na.rm=T) %>% sort %>% tail
A$MetroArea[A$Country=="Somalia"] %>% table %>% sort %>% tail
```
